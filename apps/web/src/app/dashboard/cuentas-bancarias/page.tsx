...
export default function CuentasBancariasPage() {
  ...
  const dataList = (cuentas || []).filter((c: any) => {
      ...
      <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Registra los métodos de depósito y billeteras digitales de tu negocio.</p>
          </div>
                ...
                <TableRow key={c.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-semibold text-slate-900 text-sm">{c.banco}</p>
                    </div>
                  </TableCell>
                  <TableCell>
                    <span className="text-xs text-slate-600">{c.numeroCuenta}</span>
                  </TableCell>
                  <TableCell>
                    <span className="text-xs text-slate-600">{c.tipoCuenta || 'N/A'}</span>
                  </TableCell>
                  <TableCell>
                    <span className="font-bold text-emerald-600">{c.moneda} {Number(c.saldo).toFixed(2)}</span>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Protect permission="cuentas_bancarias:actualizar" fallbackType="hide">
                        <Button variant="ghost" size="icon" onClick={() => handleOpenModal(c)} className="text-slate-500 hover:text-indigo-600">
                          <Edit className="h-4 w-4" />
                        </Button>
                      </Protect>
                      <Protect permission="cuentas_bancarias:eliminar" fallbackType="hide">
                        <Button variant="ghost" size="icon" onClick={() => { setItemToDelete(c); setConfirmOpen(true); }} className="text-slate-500 hover:text-rose-600">
